...
function showContent() {
    ...
    dayList.forEach( (item, index) => {
        ...
        for (let i = 0; i < day.length; i++) {
            ...
            list +=
                `<li class="day__item ${day[i].priority} ${day[i].status}" id=${i}>${day[i].content}
                    <label>
...
function addTask() {
    ...
    const input = document.querySelector('.list__input');
    checkInput(input);
    week[index].data.push({id: week[index].data.length, status: 'not-done', priority: priority, content: `${input.value}`});
    input.value = '';
